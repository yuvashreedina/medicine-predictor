...
with st.sidebar:
    ...
    st.markdown("---")
    page=st.radio("📍 Navigate",["🏠 Home","🔬 Safety Dashboard","💊 Drug Interaction Checker"])
    st.markdown("---")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    st.markdown("""<h2 style="color:#00a884">💊 Drug Interaction Checker</h2>""", unsafe_allow_html=True)
    st.caption("Select two medicines to instantly check if they are safe to take together")
    st.markdown("---")

    INTERACTIONS = {
        ("Warfarin","Aspirin"): {"level":"DANGEROUS","msg":"Warfarin + Aspirin together causes serious bleeding risk. Both thin the blood — combination can lead to internal bleeding or brain hemorrhage.","advice":"Never combine without strict doctor supervision and regular INR monitoring."},
        ("Warfarin","Ibuprofen"): {"level":"DANGEROUS","msg":"Ibuprofen increases Warfarin levels in blood causing dangerous bleeding risk.","advice":"Use Paracetamol instead for pain relief if on Warfarin."},
        ("Metronidazole","Warfarin"): {"level":"DANGEROUS","msg":"Metronidazole significantly increases Warfarin effect causing bleeding risk.","advice":"Monitor INR closely. Doctor may need to reduce Warfarin dose."},
        ("Diazepam","Codeine"): {"level":"DANGEROUS","msg":"Both cause heavy sedation and breathing depression. Combination can be fatal.","advice":"Never combine these two medicines. Seek immediate medical advice."},
        ("Alprazolam","Codeine"): {"level":"DANGEROUS","msg":"Extremely dangerous combination — both suppress breathing. Can cause death.","advice":"Never take together. This combination has caused many fatalities worldwide."},
        ("Metformin","Ibuprofen"): {"level":"CAUTION","msg":"Ibuprofen can reduce kidney function which affects how Metformin is removed from body — risk of lactic acidosis.","advice":"Use Paracetamol for pain if you take Metformin. Avoid long-term Ibuprofen use."},
        ("Ciprofloxacin","Theophylline"): {"level":"DANGEROUS","msg":"Ciprofloxacin increases Theophylline levels causing toxicity — seizures and heart rhythm problems.","advice":"Avoid combination. Tell your doctor about both medicines."},
        ("Sertraline","Tramadol"): {"level":"DANGEROUS","msg":"Both affect serotonin levels. Combination causes serotonin syndrome — fever, confusion, rapid heartbeat.","advice":"Never combine. Discuss with psychiatrist if both are needed."},
        ("Fluoxetine","Tramadol"): {"level":"DANGEROUS","msg":"Serotonin syndrome risk — fever, agitation, muscle twitching, rapid heart rate.","advice":"Avoid this combination completely."},
        ("Amlodipine","Lisinopril"): {"level":"MONITOR","msg":"Both lower blood pressure. Together may cause excessive BP drop causing dizziness and fainting.","advice":"Monitor blood pressure regularly. Sit up slowly after resting."},
        ("Metoprolol","Amlodipine"): {"level":"MONITOR","msg":"Both slow heart rate and lower BP. May cause excessive slowing of heart.","advice":"Regular heart rate monitoring needed. Report dizziness immediately."},
        ("Glibenclamide","Metformin"): {"level":"MONITOR","msg":"Both lower blood sugar. Together increases hypoglycemia risk.","advice":"Monitor blood sugar regularly. Always carry glucose tablets."},
        ("Insulin (Regular)","Metformin"): {"level":"MONITOR","msg":"Combined blood sugar lowering effect increases hypoglycemia risk.","advice":"Monitor blood sugar closely. Adjust doses only with doctor guidance."},
        ("Warfarin","Vitamin C"): {"level":"MONITOR","msg":"High dose Vitamin C may affect Warfarin levels.","advice":"Stick to recommended Vitamin C doses. Monitor INR."},
        ("Amlodipine","Atorvastatin"): {"level":"MONITOR","msg":"Amlodipine can increase Atorvastatin levels slightly raising muscle damage risk.","advice":"Monitor for muscle pain or weakness. Doses above 40mg Atorvastatin need caution."},
        ("Ciprofloxacin","Warfarin"): {"level":"DANGEROUS","msg":"Ciprofloxacin significantly increases Warfarin effect causing dangerous bleeding.","advice":"Avoid combination if possible. If needed, monitor INR every 2-3 days."},
        ("Disulfiram","Metronidazole"): {"level":"DANGEROUS","msg":"Both together cause severe psychotic reactions and confusion.","advice":"Never combine these medicines."},
        ("Aspirin","Ibuprofen"): {"level":"CAUTION","msg":"Both are NSAIDs. Together increases stomach bleeding risk significantly.","advice":"Avoid taking both together. Choose one only."},
        ("Codeine","Diazepam"): {"level":"DANGEROUS","msg":"Extreme sedation and breathing suppression — potentially fatal.","advice":"Never combine. This is a dangerous combination."},
        ("Metformin","Alcohol"): {"level":"DANGEROUS","msg":"Alcohol with Metformin increases lactic acidosis risk — a life-threatening condition.","advice":"Avoid alcohol completely while on Metformin."},
        ("Sertraline","Aspirin"): {"level":"CAUTION","msg":"Both increase bleeding risk. Together may cause stomach or internal bleeding.","advice":"Use Paracetamol instead of Aspirin if on Sertraline."},
        ("Fluoxetine","Aspirin"): {"level":"CAUTION","msg":"Increased bleeding risk when combined.","advice":"Use Paracetamol for pain if on Fluoxetine."},
        ("Atorvastatin","Warfarin"): {"level":"MONITOR","msg":"Atorvastatin may slightly increase Warfarin effect.","advice":"Monitor INR when starting or stopping Atorvastatin."},
        ("Theophylline","Azithromycin"): {"level":"CAUTION","msg":"Azithromycin may increase Theophylline levels causing toxicity.","advice":"Monitor for Theophylline toxicity symptoms — nausea, tremors, rapid heartbeat."},
        ("Warfarin","Clopidogrel"): {"level":"DANGEROUS","msg":"Double antiplatelet effect causes very high bleeding risk.","advice":"Only combine under strict cardiologist supervision with regular monitoring."},
    }

    all_drugs = sorted(list(DRUG_DATA.keys()))

    col1, col2 = st.columns(2)
    with col1:
        st.markdown("### 💊 First Medicine")
        drug1 = st.selectbox("Select first medicine", all_drugs, key="drug1")
    with col2:
        st.markdown("### 💊 Second Medicine")
        drug2 = st.selectbox("Select second medicine", [d for d in all_drugs if d != drug1], key="drug2")
    ...
    if st.button("🔍 Check Interaction", use_container_width=True):
        ...
        if interaction:
            ...
            colors = {"DANGEROUS":"#d32f2f","CAUTION":"#ffa000","MONITOR":"#1976d2"}
            bgs = {"DANGEROUS":"#ffebee","CAUTION":"#fff8e1","MONITOR":"#e3f2fd"}
            emojis = {"DANGEROUS":"🚨","CAUTION":"⚠️","MONITOR":"👁️"}
            c = colors[level]; b = bgs[level]; e = emojis[level]
            st.markdown(f"""<div style='background:{b};border:2px solid {c};border-radius:16px;padding:1.5rem;text-align:center'>
            <div style='font-size:3rem'>{e}</div>
            <h2 style='color:{c}'>{level} INTERACTION</h2>
            <p style='font-size:1.1rem'><b>{drug1}</b> + <b>{drug2}</b></p>
            </div>""", unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.markdown(f"<div style='background:#f3f0ff;border-left:4px solid #7c4dff;padding:1rem 1.5rem;border-radius:8px;margin:0.5rem 0'>⚠️ <b>What happens:</b> {interaction['msg']}</div>", unsafe_allow_html=True)
            st.markdown(f"<div style='background:#e8f5e9;border-left:4px solid #00c853;padding:1rem 1.5rem;border-radius:8px;margin:0.5rem 0'>💡 <b>What to do:</b> {interaction['advice']}</div>", unsafe_allow_html=True)
        else:
            st.markdown(f"""<div style='background:#e8f5e9;border:2px solid #00c853;border-radius:16px;padding:1.5rem;text-align:center'>
            <div style='font-size:3rem'>✅</div>
            <h2 style='color:#00c853'>NO KNOWN INTERACTION</h2>
            <p><b>{drug1}</b> + <b>{drug2}</b> — No major interaction found in our database</p>
            </div>""", unsafe_allow_html=True)
            st.markdown("<br>", unsafe_allow_html=True)
            st.info("ℹ️ No known serious interaction between these two medicines. However always inform your doctor about ALL medicines you take.")

        # Show both drug info side by side
        st.markdown("<br>", unsafe_allow_html=True)
        st.markdown("### 📋 Individual Drug Information")
        c1, c2 = st.columns(2)
        for col, dname in [(c1, drug1), (c2, drug2)]:
            if dname in DRUG_DATA:
                info = DRUG_DATA[dname]
                with col:
                    st.markdown(f"**{dname}** ({info['class']})")
                    st.caption(f"Usual dose: {info['usual_dose']} | Best time: {info['best_time']}")
                    for s in info["side_effects"]:
                        st.markdown(f"<div class='tip-box'>🟡 {s}</div>", unsafe_allow_html=True)

        st.markdown("<br>", unsafe_allow_html=True)
        st.success("✅ For research and educational purposes only. Always consult a certified doctor before combining medicines.")

    st.markdown("---")
    st.markdown("### ⚡ Common Dangerous Combinations to Know:")
    dangerous = [("Warfarin","Aspirin","Severe bleeding risk"),("Diazepam","Codeine","Breathing suppression — can be fatal"),("Sertraline","Tramadol","Serotonin syndrome"),("Metformin","Ibuprofen","Kidney stress + lactic acidosis"),("Ciprofloxacin","Warfarin","Dangerous bleeding")]
    for d1, d2, reason in dangerous:
        st.markdown(f"<div style='background:#ffebee;border-left:4px solid #d32f2f;padding:0.8rem 1.2rem;border-radius:8px;margin:0.3rem 0'>🚨 <b>{d1} + {d2}</b> — {reason}</div>", unsafe_allow_html=True)
